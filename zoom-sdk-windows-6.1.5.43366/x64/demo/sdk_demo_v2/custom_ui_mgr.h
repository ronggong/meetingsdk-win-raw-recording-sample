...
private:
	...
	void ShowPreview();
	void ShowWaitingRoomPreview();
	void StopPreview();
	
	void CreateToolBar();
	void ShowToolbar();
	void HandleInMeeting();
	void HandleMoveToWaitingRoom(ZOOM_SDK_NAMESPACE::MeetingStatus status);
	void DoLayout();
	...
	void UpdateMeetingTitle(ZOOM_SDK_NAMESPACE::MeetingStatus status = ZOOM_SDK_NAMESPACE::MEETING_STATUS_IDLE);
