...
void CMoreMenuUIMgr::Notify( TNotifyUI& msg )
{
	if (msg.sType == _T("itemclick"))
	{
		...
		{
			time_t timeNow;
			ZOOM_SDK_NAMESPACE::SDKError err = CustomizedUIRecordMgr::GetInstance()->CanIStartLocalRecording();
			if (err == ZOOM_SDK_NAMESPACE::SDKERR_NO_PERMISSION){
				::MessageBox(NULL, _T("please request the permission from the host"), _T("winsdk demo"), MB_OK);
				return;
			}
			if (err == ZOOM_SDK_NAMESPACE::SDKERR_WRONG_USAGE) {
				::MessageBox(NULL, _T("this is the wrong usage"), _T("winsdk demo"), MB_OK);
				return;
			}
			time(&timeNow);
		...
		{
			ZOOM_SDK_NAMESPACE::SDKError err = CustomizedUIRecordMgr::GetInstance()->CanIStartCloudRecording();
			if (err == ZOOM_SDK_NAMESPACE::SDKERR_NO_PERMISSION) {
				::MessageBox(NULL, _T("please request the permission from the host"), _T("winsdk demo"), MB_OK);
				return;
			}
