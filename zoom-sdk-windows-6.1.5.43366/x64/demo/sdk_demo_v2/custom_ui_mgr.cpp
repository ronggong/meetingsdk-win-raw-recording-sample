...
#include "meeting_service_components/meeting_whiteboard_ctrl_interface.h"
//#include <atlbase.h>
...
void CCustomizeInMeetingUIMgr::ShowWaitingRoomPreview()
{
	if (m_bUIIsReady && m_pCustomizeUIVideoMgr)
	{
		::GetClientRect(m_hOwnerHwnd, &m_rcVideo);
		m_videoSettingsWorkFlow.SetVideoPreviewParentWnd(m_hOwnerHwnd, m_rcVideo);
...
void CCustomizeInMeetingUIMgr::onMeetingStatusChanged(ZOOM_SDK_NAMESPACE::MeetingStatus status, int iResult /*= 0*/)
{
	...
	case ZOOM_SDK_NAMESPACE::MEETING_STATUS_IN_WAITING_ROOM:
		HandleMoveToWaitingRoom(status);
		break;
	default:
		break;
	}
...
void CCustomizeInMeetingUIMgr::HandleInMeeting()
{
	...
	if (m_pMeetingService && m_pMeetingService->GetMeetingWhiteboardController())
	{
		m_pMeetingService->GetMeetingWhiteboardController()->SetOwnerWindow(m_hOwnerHwnd);
	}	
}

void CCustomizeInMeetingUIMgr::HandleMoveToWaitingRoom(ZOOM_SDK_NAMESPACE::MeetingStatus status)
{
	if (m_pCustomUIMgr)
	{
		DestroyAllSubWindow();
		...
		if(m_bUIIsReady)
			ShowWaitingRoomPreview();
		UpdateMeetingTitle(status);
	}
}
...
void CCustomizeInMeetingUIMgr::ShowWhiteboardWindow()
{
	if (!m_pMeetingService || !m_pMeetingService->GetMeetingWhiteboardController())
		return;

	m_pMeetingService->GetMeetingWhiteboardController()->ShowDashboardView();
}
...
void CCustomizeInMeetingUIMgr::UpdateMeetingTitle(ZOOM_SDK_NAMESPACE::MeetingStatus status)
{
	...
	if(m_hOwnerHwnd && IsWindow(m_hOwnerHwnd))
	{
		if (status == ZOOM_SDK_NAMESPACE::MEETING_STATUS_IN_WAITING_ROOM)
			...
			swprintf_s(strTitle, title_len, _T("zoom_sdk_demo_ui -- You are waiting for the host"));
