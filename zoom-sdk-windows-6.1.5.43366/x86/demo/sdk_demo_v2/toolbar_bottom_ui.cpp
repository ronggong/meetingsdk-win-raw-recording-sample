...
void CToolbarBottom::InitWindow()
{
	...
	m_btnWhiteboard = static_cast<CButtonUI*>(m_PaintManager.FindControl(_T("btn_whiteboard")));
	if (m_btnWhiteboard)
		m_btnWhiteboard->SetText(L"Whiteboards");
...
void CToolbarBottom::Notify(TNotifyUI& msg)
{
	if(msg.sType == _T("click"))
	{
		...
		*/
		...
		else if (msg.pSender == m_btnWhiteboard)
		{
			DoWhiteboardButtonClick();
		}
...
void CToolbarBottom::DoWhiteboardButtonClick()
{
	if (m_pToolbarMgr)
		m_pToolbarMgr->ShowWhiteboardWindow();
}
